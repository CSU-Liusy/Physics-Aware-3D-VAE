"""
plot_comparison.py - 全局三维重建对比矩阵图生成脚本

目标：
    生成论文用横向长图，对比同一批矿体样本在不同方法下的重建效果。

列布局（固定）：
  Ground Truth | Sparse Input | IK | 3D U-Net | Ours

特性：
    1) 默认混合选样：2 个真实矿体 + 2 个虚拟矿体（总行数默认 4）
         （可通过 --real-rows / --virtual-rows 调整）
  2) 每一行代表一个矿体样本，同一行使用同色系以便区分样本
  3) 若未找到 U-Net 权重，可自动训练一个基础 U-Net（默认 10 轮）后继续
"""
...
def _render_uncertainty_variance(ax,
    ...
    # 使用 MC 平均概率作为主渲染量，展示更大地下概率空间；
    # 同时利用方差参与采样优先级，保留不确定性信息。
...
def _safe_torch_load(path: str, map_location='cpu'):
    """
    优先使用 weights_only=True 以避免 FutureWarning 并提升安全性；
    在旧版本 PyTorch 或特殊 checkpoint 不兼容时自动回退。
    """
    try:
...
def generate_local_detail_figure(args):
    ...
    # 2x3 布局：
    # 上排：GT | 3D U-Net | Ours（全局）
    # 下排：GT Zoom | 3D U-Net Zoom | Ours Zoom（同一区域局部放大）

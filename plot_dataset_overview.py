"""
plot_dataset_overview.py - 数据集样例与分布展示图生成脚本

生成一张论文级别的数据集概览图：
  左侧：从 5000 个虚拟矿体中选 9 个形态各异的样本，渲染 3D 实体网格
  右侧：从真实矿体中选 6 个，展示半透明矿体 mesh + 红色钻孔线段叠加

使用示例：
    python 3dvae/plot_dataset_overview.py
    python 3dvae/plot_dataset_overview.py --model-name my_best.pth
    python 3dvae/plot_dataset_overview.py --virtual-dir data/mining_ply_pretrain --real-dir data/mining_ply
    python 3dvae/plot_dataset_overview.py --model-name best_model.pth --out-dir results/figures

参数说明：
    --model-dir     模型权重目录 (默认: results/model)
    --model-name    模型权重文件名，仅文件名部分，例如 best_model.pth (默认: best_model.pth)
    --virtual-dir   虚拟矿体 PLY 目录 (默认: data/mining_ply_pretrain)
    --real-dir      真实矿体 PLY 目录 (默认: data/mining_ply)
    --out-dir       输出目录 (默认: results/figures)
    --n-virtual     左侧展示的虚拟矿体数量 (默认: 9, 按 3×3 排列)
    --n-real        右侧展示的真实矿体数量 (默认: 6, 按 3×2 排列)
    --num-holes     真实矿体模拟钻孔数 (默认: 8)
    --samples-per-hole  每个钻孔采样点数 (默认: 12)
    --dpi           输出分辨率 (默认: 300)
    --seed          随机选样种子 (默认: 42)
    --size-thresh   真实矿体选样文件大小分位数 [0.0~1.0]；越大越选大文件（复杂矿体），默认 0.8
"""
...
# ── 自动将 3dvae 目录加入模块搜索路径 ──────────────────────────────────────
_SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
if _SCRIPT_DIR not in sys.path:
    sys.path.insert(0, _SCRIPT_DIR)

from dataset import read_ply  # 复用项目内自写 PLY 解析器
...
def _get_cjk_font_prop():
    """返回系统中可用的 CJK 字体 FontProperties，找不到返回 None。"""
...
def _normalize_verts(verts: np.ndarray) -> np.ndarray:
    """将顶点坐标归一化到 [0, 1]^3，实现坐标脱敏。"""
    lo = verts.min(axis=0)
    hi = verts.max(axis=0)
    span = hi - lo
    span[span < 1e-8] = 1.0  # 防零除
    return (verts - lo) / span


def _scan_ply_sizes(ply_files: list[str], max_scan: int = 500, seed: int = 42) -> list[tuple[str, int]]:
    """
    从文件列表中随机抽样 max_scan 个，快速统计顶点数作为形态差异代理指标。
    返回 [(path, num_verts), ...] 按顶点数升序排列。
    """
...
def _select_diverse(ply_files: list[str], n: int, seed: int = 42) -> list[str]:
    """
    从 PLY 列表中按顶点数分布均匀选取 n 个，以覆盖形态多样性。
    对大文件集先随机抽 500 个再分位数采样，避免全量读取。
    """
    if len(ply_files) <= n:
        return ply_files
...
def _select_real_diverse(ply_files: list[str], n: int, seed: int = 42,
    ...
    """
    专为真实矿体设计的选样函数，以磁盘文件大小作为矿体复杂度代理指标。

    size_thresh 语义（0.0 ~ 1.0）：
      - 控制在文件大小排序中的采样窗口中心分位数
      - 1.0 → 窗口居于最大文件端（优先选最大/最复杂矿体）
      - 0.0 → 窗口居于最小文件端（优先选最小矿体）
      - 0.5 → 居中采样
      - 推荐：0.7~1.0，以获得轮廓清晰、凹凸丰富的展示样本

    采样逻辑：
      1. 按文件大小升序排列全部 PLY
      2. 以 size_thresh 为中心分位数，取宽度为 max(3n, N*0.4) 的窗口
      3. 在窗口内按等间隔分位数选取 n 个，保证多样性
    """
    if len(ply_files) <= n:
        return ply_files

    # 按文件大小排序（全量，O(N) stat，无需读取内容）
...
def _simulate_drillholes(verts: np.ndarray,
    ...
    """
    在归一化 [0,1]^3 坐标下模拟竖直钻孔轨迹。
    每个钻孔 = 一条垂直线（在 z 方向从顶到底均匀采样）。
    返回 [(N,3), ...] 每个元素是一个钻孔轨迹点序列。
    """
...
def _collect_ply(directory: str, label: str) -> list[str]:
    """收集目录下所有 .ply 文件，目录不存在或为空时给出提示并退出。"""
...
def generate_figure(args) -> None:
    ...
    # 注意：本脚本不加载模型推理，仅作数据集可视化展示。
    # 若后续需要添加模型重建结果展示，可在此处加载 model_path 中的权重。
    ...
    # ── 4. 布局设计 ─────────────────────────────────────────────────────────
    # 左侧：虚拟矿体 n_rows_v × n_cols_v
    # 右侧：真实矿体 n_rows_r × n_cols_r
    # 左右权重比 ≈ 3:2
    nv = args.n_virtual
    nr = args.n_real
    ...
    # ── 11. 自动执行定量评估（生成 CSV/Markdown + 控制台打印）────────────────
    # ── 10.8. 联动生成认知不确定性方差热力图 ──────────────────────────────
